...
public class MyClassLoader extends ClassLoader {
    ...
    protected Class<?> findClass(String name) throws ClassNotFoundException {
        ...
        try {
            byte[] byteList = readByteList(name);
            klass = defineClass(name, byteList, 0, byteList.length);
            return klass;
        } catch (Exception e) {
    ...
    private byte[] readByteList(String className) throws IOException {
        return Files.readAllBytes(new File(bytecodeFileDirectory, className + ".class").toPath());
    }
