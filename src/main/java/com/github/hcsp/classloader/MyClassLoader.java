...
public class MyClassLoader extends ClassLoader {
    ...
    protected Class<?> findClass(String name) throws ClassNotFoundException {
        byte[] bytecodeFileDirectoryData;
        try {
            bytecodeFileDirectoryData = loadClassData(name);
        } catch (IOException e) {
            throw new ClassNotFoundException(name);
        }
        return this.defineClass(name, bytecodeFileDirectoryData, 0, bytecodeFileDirectoryData.length);
    }
    private byte[] loadClassData(String name) throws IOException {
        return Files.readAllBytes(new File(name + ".class").toPath());
    }
