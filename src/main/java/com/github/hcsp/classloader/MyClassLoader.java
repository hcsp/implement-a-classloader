...
public class MyClassLoader extends ClassLoader {
    ...
    protected Class<?> findClass(String name) throws ClassNotFoundException {
        ...
        try {
            fileContent = Files.readAllBytes(new File(this.bytecodeFileDirectory.getPath(), name + ".class").toPath());
        } catch (IOException e) {
            throw new ClassNotFoundException(name);
        }
        return this.defineClass(name, fileContent, 0, fileContent.length);
    }
